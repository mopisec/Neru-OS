...
void NeruMain(void)
{
	...
	/* ファイル読み込み */
	i = api_fopen(p); if (i == 0) { error("Error: File not found.\n"); }
	fsize = api_fsize(i, 0);
	if (fsize > 512 * 1024) {
	...
	if (info_BMP(&env, info, fsize, filebuf) == 0) {
		...
		if (info_JPEG(&env, info, fsize, filebuf) == 0) {
			/* JPEGでもなかった */
			api_putstr0("Error: File type unknown.\n");
			api_end();
		}
	}
	...
	sprintf(tit, "Image View (%s)", p);
	win = api_openwin(winbuf, xsize, info[3] + 37, -1, tit);
