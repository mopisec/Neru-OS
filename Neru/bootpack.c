...
void NeruMain(void)
{
	...
	*((int *) 0x0fe4) = (int) shtctl;
	...
	mx = (binfo->scrnx - 11) / 2; /* ���ʒ����ɂȂ��悤�ɍ��W�v�Z */
	...
	*((int *) 0x0fe8) = (int) nihongo;
	...
	/* startup */
	finfo = file_search("startup.hsf", (struct FILEINFO *) (ADR_DISKIMG + 0x002600), 224);
	if (finfo != 0) {
		char *ss;
		i = finfo->size;
	...
	for (;;) {
		...
		if (fifo32_status(&fifo) == 0) {
			...
			if (256 <= i && i <= 511) { /* �L�[�{�[�h�f�[�^ */
				...
				memman_free_4k(memman, (int) sht2->buf, 256 * 660);
				sheet_free(sht2);
			}
		}
	}
...
struct SHEET *open_console(struct SHTCTL *shtctl, unsigned int memtotal)
{
	...
	unsigned char *buf = (unsigned char *) memman_alloc_4k(memman, 256 * 660);
	sheet_setbuf(sht, buf, 256, 660, -1); /* �����F�Ȃ� */
	make_window8(buf, 256, 660, "Console", 0);
	//����: x = 256 , y = 165
	make_textbox8(sht, 8, 28, 240, 623, COL8_000000);
...
void close_console(struct SHEET *sht)
{
	...
	memman_free_4k(memman, (int) sht->buf, 256 * 660);
	sheet_free(sht);
	close_constask(task);
	return;
}
